...
fn list_encrypted_headers() {
    let mut entries = Archive::with_password("data/comment-hpw-password.rar", "password").unwrap()
        .list().unwrap();
    assert_eq!(entries.next().unwrap().unwrap().filename, PathBuf::from(".gitignore"));
}
...
fn no_password_list_encrypted_headers() {
    ...
    let mut entries = Archive::new("data/comment-hpw-password.rar").unwrap()
        .list().unwrap();
...
fn extract_encrypted_headers() {
    let t = TempDir::new("unrar").unwrap();
    Archive::with_password("data/comment-hpw-password.rar", "password").unwrap()
        .extract_to(t.path())
        .unwrap()
        .process()
        .unwrap();
